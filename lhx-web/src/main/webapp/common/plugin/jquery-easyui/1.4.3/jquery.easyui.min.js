...
$(this).find("div.progressbar-text").html("");
if(_1e7!=_1e6){
...
if(opts.modal==true){
_27f.mask=$("<div class=\"window-mask\" style=\"display:none\"><iframe name=\"window-mask-iframe\" style=\"position: absolute; z-index: -1; width: 100%; height: 100%; top: 0;left:0;background-color:transparent\" scrolling'\"no\" frameborder=\"0\" allowTransparency=\"true\"></iframe></div>").insertAfter(_27f.window);
}
if(_27f.shadow){
_27f.shadow.remove();
}
...
//add by zhanghongxing on 20160602, the code is used to delete end of data where it is 0.  code start
if(val.indexOf(".")>0){
	val = val.replace(/0+?$/, "");
	val = val.replace(/[.]$/, "");
}
	...
	$.ajax=function(opt){
		...
		var _opt = $.extend(opt,{
			error:function(XMLHttpRequest, textStatus, errorThrown){
				//错误方法增强处理
				try{
					errorAlert('当前网络不稳定，请刷新浏览器后重试！\r\n错误信息：'+XMLHttpRequest.status+XMLHttpRequest.statusText);
				} catch (e){
					alert('当前网络不稳定，请刷新浏览器后重试！\r\n错误信息：'+XMLHttpRequest.status+XMLHttpRequest.statusText);
				}
