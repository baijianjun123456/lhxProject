package com.lhx.common.mybatis.pagination;

import java.util.List;

/**
 * 查询参数对象
 * @author liangshu
 */
public class PageModel<T> {
	
	static class Pagination{
		public static final Integer DEFAULT_START = 1;
		
		public static final Integer PAGE_SHOW_NUM = 10;
	}
	
	public static final Integer NO_SQL_COUNT = -1;

	/**页号*/
	private Integer pageNo = Pagination.DEFAULT_START;
	
	/**每页显示记录数*/
	private Integer pageSize = Pagination.PAGE_SHOW_NUM;
	
	/**是否使用分页查询*/
	private boolean usePage = true;
	
	/**查询的排序字段集合*/
	private String orderbyInfo ;
	
	public String getOrderbyInfo() {
		return orderbyInfo;
	}

	public void setOrderbyInfo(String orderbyInfo) {
		this.orderbyInfo = orderbyInfo;
	}

	/**查询的结果总页数*/
	private Integer totalPageNum;

	/**查询的结果总记录数*/
	private Integer total;
	
	/**查询的结果集合*/
	private List<T> rows;

	public Integer getTotalPageNum() {
		return totalPageNum;
	}

	/** 
	 * 获取从查询询结果记录中开始提取记录的顺序号
	 * @return 
	 */
	public Integer getStart() {
		return (pageNo-1)*pageSize;
	}

	/** 
	 * 获取查询提取记录的个数
	 * @return 
	 */
	public Integer getCount() {
		return pageSize;
	}
	
	/**
	 * 获取页号
	 */
	public Integer getPageNo() {
		return pageNo;
	}

	/**
	 * 获取每页记录数
	 */
	public Integer getPageSize() {
		return pageSize;
	}

	/**
	 * 获取是否使用分页
	 * @return
	 */
	public boolean isUsePage() {
		return usePage;
	}
	
	/** 
	 * 获取查询总记录数
	 * @return 
	 */
	public Integer getTotal() {
		return total;
	}
	
	/** 
	 * 获取查询结果集合
	 * @return 
	 */
	public List<T> getRows() {
		return rows;
	}
	
	/**
	 * 设置页号
	 */
	public void setPageNo(Integer pageNo) {
		this.pageNo = pageNo;
	}

	/**
	 * 设置每页记录数
	 */
	public void setPageSize(Integer pageSize) {
		this.pageSize = pageSize;
	}

	/**
	 * 设置是否使用分页
	 */
	public void setUsePage(boolean usePage) {
		this.usePage = usePage;
	}

	/**
	 * 设置查询总记录数
	 * @param totalNum 查询总记录数
	 */
	public void setTotal(Integer total) {
		this.total = total;
		if(pageSize==0){
			pageSize = Pagination.PAGE_SHOW_NUM;
		}
		this.totalPageNum = total%pageSize==0?total/pageSize:total/pageSize+1;
	}
	
	/**
	 * 设置查询的结果集合
	 * @param queryResult 查询结果集合
	 */
	public void setRows(List<T> Rows) {
		this.rows = Rows;
	}

	public void setTotalPageNum(Integer totalPageNum) {
		this.totalPageNum = totalPageNum;
	}
}